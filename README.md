...
Secrets are intended to flow through AWS SSM Parameter Store and External Secrets, not plaintext Kubernetes Secret manifests. Do not commit credentials, kubeconfigs, `.env` files, AWS keys, database passwords, or generated Terraform state.

## Notes

- Root README is overview only. See `src/README.md` and `k8s-infra-aws-ssm/README.md` for deeper app and Kubernetes notes.
- Validate AWS identity, region, cluster access, and ECR repository names before running deployment scripts.
