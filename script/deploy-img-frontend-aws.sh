...
if [[ "${ENV}" == "all" ]]; then
  echo "==> Deploying frontend ALL environments: dev, then uat"
  "${BASH_SOURCE[0]}" dev "$@"
  "${BASH_SOURCE[0]}" uat "$@"
  echo "==> ALL environments frontend deployed (dev + uat)"
  exit 0
fi
...
case "${ENV}" in
  ...
  uat)
    ...
    REPOSITORY_NAME="${FRONTEND_REPOSITORY_NAME:-uat-demo-frontend}"
    ;;
esac
