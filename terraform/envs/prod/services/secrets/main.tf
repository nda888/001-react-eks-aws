...
data "terraform_remote_state" "eks_alb" {
  ...
  config = {
    bucket = var.state_bucket
    key    = "prod/services/eks-alb/terraform.tfstate"
    region = var.aws_region
  }
}
...
# ---------------------------------------------------------------------------
# Namespace guard: app namespace must exist (owned by eks-alb)
# ---------------------------------------------------------------------------
...
resource "random_password" "monitor_loki_writer_token" {
  length  = 32
  special = false
}

resource "random_password" "monitor_prometheus_rw_token" {
  length  = 32
  special = false
}
...
resource "aws_ssm_parameter" "monitor_loki_writer_token" {
  name  = "${var.monitor_ssm_prefix}/loki_writer_token"
  type  = "SecureString"
  value = random_password.monitor_loki_writer_token.result
  tags  = local.common_tags
}

resource "aws_ssm_parameter" "monitor_prometheus_rw_token" {
  name  = "${var.monitor_ssm_prefix}/prometheus_rw_token"
  type  = "SecureString"
  value = random_password.monitor_prometheus_rw_token.result
  tags  = local.common_tags
}
