...
resource "aws_security_group" "alb_frontend" {
  name        = "eks-react-prod-alb-frontend"
  name_prefix = null
...
resource "aws_security_group" "alb_backend" {
  name        = "eks-react-prod-alb-backend"
  name_prefix = null
...
# --- Prod namespace (prerequisite for environment resources) ---
resource "kubernetes_namespace" "prod" {
  metadata {
    name = "prod"
  }

  # Must destroy namespace BEFORE ALB controller is killed.
  # Ingress + TargetGroupBindings have ALB controller finalizers that
  # require a running controller to clear. Reverses the default
  # terraform destroy ordering which destroyed helm_release first.
