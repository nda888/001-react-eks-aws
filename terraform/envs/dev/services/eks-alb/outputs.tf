...
output "next_steps" {
  ...
  value       = <<-EOT
     ...
     1. Apply the AWS Kubernetes manifests:
         kubectl apply -k k8s-infra-aws-ssm-dev-uat/
     ...
     4. Get the ALB DNS name (via AWS CLI):
         aws elbv2 describe-load-balancers --region us-east-1 --names elb-react-eks-dev-uat --query 'LoadBalancers[0].DNSName' --output text
