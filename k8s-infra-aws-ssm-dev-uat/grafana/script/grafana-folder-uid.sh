...
validate_admin_credentials() {
  admin_get "/api/user" >/dev/null 2>&1 \
    || fail "Grafana admin credentials invalid; provide GRAFANA_ADMIN_PASSWORD or GRAFANA_API_TOKEN"
}
...
ensure_folder_tree() {
  ...
  grafana_get "${token}" "${FOLDER_API_PATH}" >"${folders_file}"
  prod_child_uid="$(folder_uid_by_title_and_parent "${GRAFANA_PROD_CHILD_FOLDER_TITLE}" "${parent_uid}" "${folders_file}")"
  if [[ -z "${prod_child_uid}" ]]; then
    log "creating child folder ${GRAFANA_PROD_CHILD_FOLDER_TITLE} under ${GRAFANA_PARENT_FOLDER_TITLE}"
    prod_child_uid="$(create_folder "${token}" "${GRAFANA_PROD_CHILD_FOLDER_TITLE}" "prod-" "${parent_uid}")"
  else
    log "found child folder ${GRAFANA_PROD_CHILD_FOLDER_TITLE}"
  fi
